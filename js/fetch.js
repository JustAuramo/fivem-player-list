...
export const fetchServer = (serverId) => {
	try {
		if (!isValidServerId(serverId)) {
			showNotification('Annoit väärässä muodossa palvelimen ID', 'error');
			return;
		}

		setTitle('Ladataan palvelimen tietoja seuraavalla tavalla: FiveM API...');

		showLoader(true);

		refreshButton.onclick = () => fetchServer(serverId);
		const url = `${API_BASE_URL}/servers/single/${serverId}`;
		console.info(`Haetaan palvelimen tietoja`, serverId, url);
		...
		retryFetch(url, { headers: DEFAULT_HEADERS })
			...
			.then((json) => {
				...
				showNotification('Palvelimen tiedot ladattu onnistuneesti', 'success');
			})
			.catch((error) => {
				console.error(error);
				setTitle('Virhe ladatessa palvelimen tietoja');
				showNotification('Palvelimen tietojen lataus epäonnistui', 'error');
				showLoader(false);
			});
	} catch (error) {
		console.error('Virhe palvelimen tietojen latauksessa:', error);
		showNotification('Tapahtui odottamaton virhe', 'error');
		showLoader(false);
	}
};

const fetchPlayers = (url, playersFetch = false) => {
	console.info('Pelaajien hakeminen metodilla:', playersFetch ? 'players.json' : 'normal', url);
	retryFetch(url, { headers: DEFAULT_HEADERS })
		.then(handleResponse)
		.then((json) => {
		...
		.catch((error) => {
			console.error(error);
			showNotification('Pelaajien tietojen lataus epäonnistui', 'error');
			showLoader(false);
		});
};
...
export const renderPlayers = (players, search = false) => {
	...
	players.forEach((player) => {
		...
		starImg.alt = isFavorite ? 'Remove from Favorites' : 'Lisää suosikkeihin';
		starImg.title = isFavorite ? 'Remove from Favorites' : 'Lisää suosikkeihin';
	...
	span1.textContent = 'Tämä sivu ei ole millään tavalla sidoksissa FiveM alustaan tai mihinkään muuhun palvelimeen.';
	footerTd.appendChild(span1);
	footerTd.appendChild(document.createElement('br'));

	const span2 = document.createElement('span');
	span2.appendChild(document.createTextNode('Muokannut: '));

	const link = document.createElement('a');
	link.href = 'https://github.com/JustAuramo';
	link.target = '_blank';
	link.rel = 'noopener noreferrer';
	link.textContent = 'Auramo';
